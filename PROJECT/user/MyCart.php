        ...
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            ...
            $res = $con->query($sel);
                while ($row=$res->fetch_assoc()) {
                    $selPr = "select * from tbl_plant where plant_id='" .$row["plant_id"]. "'";
                    $respr = $con->query($selPr);
                    if ($rowpr=$respr->fetch_assoc()) {
                        $selstock = "select sum(stock_quantity) as stock from tbl_stock where plant_id='".$rowpr["plant_id"]."'";
						$selstock1="select sum(cart_quantity) as quantity from tbl_cart where plant_id='".$rowpr["plant_id"]."' and cart_status >'0'";
            ...
            <div class="product">
                <div class="product-image">
                    <img
                        src="../Assets/File/Plant/<?php echo $rowpr["plant_photo"] ?>"
                        />
                </div>
                <div class="product-details">
                    <div class="product-title"><?php echo $rowpr["plant_name"] ?></div>
                    <p class="product-description">
                    <?php echo $rowpr["plant_description"] ?>
                    </p>
                </div>
                <div class="product-price "	><?php echo $rowpr["plant_price"] ?></div>
